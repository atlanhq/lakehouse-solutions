"""
PySpark + Iceberg connection verification script for GCS via an HTTP/S proxy.

Identical to pyspark_lakehouse_gcs.py (lists namespaces, table counts, gold.assets
row count) except that proxy settings are read from standard environment variables
and injected via both JVM system properties and Iceberg's REST catalog client config
(rest.client.proxy.hostname / rest.client.proxy.port).
...
── usage ─────────────────────────────────────────────────────────────────────
    python pyspark_lakehouse_gcs_proxy.py
...
── environment variables ─────────────────────────────────────────────────────
    ...
    Proxy (at least one required):
    https_proxy / HTTPS_PROXY   e.g. http://proxy.corp.example.com:8080
...
def create_spark_session():
    """Create a SparkSession wired to the Polaris REST catalog on GCS via proxy."""
    ...
    if not proxy_host or proxy_port == "None":
        raise ValueError(
            "No proxy configured — set https_proxy, HTTPS_PROXY, http_proxy, or HTTP_PROXY"
        )
    ...
    builder = (
        ...
        .appName("pyspark-lakehouse-gcs-proxy-verify")
        .master("local[*]")
        ...
        # ── Proxy (JVM system properties for Spark's HTTP client) ─────────────
        ...
        .config(f"spark.sql.catalog.{CATALOG_NAME}.header.X-Iceberg-Access-Delegation",
                "vended-credentials")
        .config(f"spark.sql.catalog.{CATALOG_NAME}.rest.client.proxy.hostname", proxy_host)
        .config(f"spark.sql.catalog.{CATALOG_NAME}.rest.client.proxy.port",     proxy_port)
...
def main():
    logger.info("=" * 65)
    logger.info("PySpark Lakehouse GCS (proxy) — Connection Verification")
    logger.info("=" * 65)
